...
@Injectable()
export class demandeCongeProvider {
  dataconge: any;
  // dataPoint : any;
 ...
 loadconge() {
  if (this.dataconge) {
    // already loaded data
    return Promise.resolve(this.dataconge);
  }
  ...
  return new Promise(resolve => {
    ...
    this.http.get('http://testmariadb.alwaysdata.net/public/demandeConge/demandeConge')
      .map(res => res.json())
      .subscribe(data => {
        // we've got back the raw data, now generate the core schedule data
        // and save the data for later reference
        this.dataconge = data;
        resolve(this.dataconge);
      });
  });
}
...
saveconge(data) {
  return new Promise((resolve, reject) => {
    this.http.post('http://testmariadb.alwaysdata.net/public/demandeConge/demandeConges',data)
      .subscribe(res => {
        resolve(res);
      }, (err) => {
        reject(err);
...
edit(id,postInfo){
  return new Promise((resolve, reject) => {
   this.storage.get('token').then((value) => {
     ...
     this.http.put('http://testmariadb.alwaysdata.net/public/demandeConge/demandeConge/' +id ,  JSON.stringify(postInfo),  {headers: headers})
       .subscribe(data => {
         resolve(data);
       }, (err) => {
         reject(err);
