...
export class PointageProvider {
 ...
 loadpointage() {
  if (this.datapointage) {
    // already loaded data
    return Promise.resolve(this.datapointage);
  }
  ...
  return new Promise(resolve => {
    ...
    this.http.get('http://testmariadb.alwaysdata.net/public/pointage/pointage')
      .map(res => res.json())
      .subscribe(data => {
        // we've got back the raw data, now generate the core schedule data
        // and save the data for later reference
        this.datapointage = data;
        resolve(this.datapointage);
      });
  });
}
...
savePointage(data) {
  return new Promise((resolve, reject) => {
    this.http.post('http://testmariadb.alwaysdata.net/public/pointage/pointages',data)
      .subscribe(res => {
        resolve(res);
      }, (err) => {
        reject(err);
...
edit(id,postInfo){
  return new Promise((resolve, reject) => {
   this.storage.get('token').then((value) => {
     ...
     this.http.put('http://testmariadb.alwaysdata.net/public/pointage/pointage/' +id ,  JSON.stringify(postInfo),  {headers: headers})
       .map(res => res.json())
       .subscribe(data => {
         resolve(data);
       }, (err) => {
